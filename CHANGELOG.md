...
- BatterySoHDashboard: Add null guards on p.soh_pct and p.capacity_kwh in chartData mapping to prevent TypeError when API returns null values.
...
- Alembic migrations: battery_soh_estimates table chain (4c5c9e5b4a60, d4cb158bda0c stub, f36d25e55dd8).
...
### Fixed 🛠
- **Smart Polling Data Drain**: Reverted the Target SoC tracking logic while parked. The previous update forced the collector to aggressively query the charging API every 5 minutes while the car was sleeping, defeating the purpose of Smart Polling and consuming excessive data. Parked mode now strictly relies on the single lightweight connection status check.
